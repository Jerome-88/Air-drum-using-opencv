...
MAIN_WIN = "Virtual Drum Hero - New Layout"
...
def open_camera() -> cv2.VideoCapture:
    ...
    if not cap.isOpened():
        ...
        print("[ERROR] Then run: conda activate cv && python rhythm_drum_new.py")
        sys.exit(1)
...
def handle_key(key: int, game: RhythmGame, ui: GameUI, detector: MarkerDetector, preset_name: str, show_mask: bool) -> tuple[bool, str, bool]:
    if key == 255:
...
def main() -> None:
    ...
    try:
        while running:
            ...
            running, preset_name, show_mask = handle_key(key, game, ui, detector, preset_name, show_mask)
    finally:
        cap.release()
        audio.quit()
